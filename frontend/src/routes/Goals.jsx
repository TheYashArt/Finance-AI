...
const Goals = () => {
    ...
    const handleContributeClick = (goal) => {
        ...
        if (categories.length > 0) {
            setContributeCategoryId(categories[0].id);
        }
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-card-bg border border-white/10 rounded-2xl w-full max-w-md p-6 relative shadow-2xl">
                        ...
                        <form onSubmit={handleContributeSubmit} className="space-y-4">
                            ...
                            {/* <div>
                                <label className="block text-xs text-gray-400 mb-1">Category for Expense</label>
                                <select
                                    value={contributeCategoryId}
                                    onChange={(e) => setContributeCategoryId(e.target.value)}
                                    className="w-full bg-dark-bg border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-primary/50 appearance-none"
                                    required
                                >
